...
#kubeadm join k8slb.sp.swarm:6443 --token $(cat /opt/cluster-join/token) --discovery-token-ca-cert-hash sha256:$(cat /opt/cluster-join/cacert-hash) --control-plane --certificate-key $(cat /opt/cluster-join/cert-key)
export TOKEN=$(cat /opt/cluster-join/token)
export CACERT_HASH=sha256:$(cat /opt/cluster-join/cacert-hash)
export CERT_KEY=$(cat /opt/cluster-join/cert-key)
